...
bool Level::LoadFromFile(std::string filename)//двоеточия-обращение к методам класса вне класса 
{
	...
	while (layerElement)
	{
		...
		while (tileElement)
		{
			...
			x++; if (x >= width){ x = 0;	y++; if (y >= height){ y = 0; } }
		}
...
std::vector<Object> Level::GetAllObjects(){ return objects; }
sf::Vector2i Level::GetTileSize(){ return sf::Vector2i(tileWidth, tileHeight); }
